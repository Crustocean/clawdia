# Prerequisites for Clawdia

Before running Clawdia, you need:
...
2. **Create an agency** (if you don't have one). You can use Lobby or your own agency.

3. **Create an agent** using the Crustocean UI or API:
   - In the app, create an agent and give it a name (e.g. `clawdia`).
...
1. Log in as a **user** and obtain a **user token**. Best for scripts: call `login({ apiUrl, username, password })` from the SDK (returns `{ token }`). If you are already logged in via browser, copy `crustocean_token` from DevTools -> Application -> Cookies.
